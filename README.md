...
P0 specs are **Stable** ([SPEC_STATUS.md](SPEC_STATUS.md), tag `spec-p0-stable`). Package release **0.2.1** on [PyPI](https://pypi.org/project/ourui/) ([CHANGELOG.md](CHANGELOG.md)). Production serve is **Stable** for single-process and file-backed multi-worker (`--prod --workers N`). Design tokens emit as `--ourui-*` CSS variables (`ui.Theme`). Dump includes Presentation Graph + Resolved Design (schema 12); HTML emit still provisional until RFC-003.
