"""OurUI marketing landing — dogfood (header → hero → proof → CTA → footer).

Entry for ``ourui serve`` / ``dump`` / ``emit``. OurUI compiles this file's AST
only, so page helpers are defined here. Mirror modules under ``components/`` and
``pages/`` stay in sync for folder structure.
"""
...
def hero():
    return ui.Hero(
        ...
        children=[
            ui.Code(HERO_SNIPPET, language="python"),
        ],
    )
...
def gallery():
    ...
    pad_x="6",
            ...
            ui.Shell(
                ...
                ui.Link("Language vs kit", href=f"{GITHUB}/blob/main/docs/decisions/ADR-014-language-primitives-vs-kit.md"),
                layout="row",
                gap="4",
                wrap="wrap",
            ),
