...
**RFC-003** (Draft): **Host Contract** — Generation 3; web emit still provisional until Implemented.
...
| 3 — Host (`RTR + Resolved Design` via Host Contract) | Next (RFC-003 Draft) |
...
| HTML emitter | Stable (`ourui emit`) — **provisional** vs Host Contract |
| CSS emit | Stable (inlined + bundle; `--ourui-*` tokens) — provisional vs Resolved Design |
...
| Package (`ourui` 0.2.1) | Stable ([PyPI](https://pypi.org/project/ourui/); dump schema 12 + `resolved_design`) |
| Presentation Graph | Stable (RFC-001 Option A — lowering) |
| Resolved Design | Stable (RFC-002 — dump artifact; emit does not consume yet) |
| Design System pack (`ourui-default`) | Draft (seeded from `theme.py`; packs API may grow) |
| Host Contract | Draft (RFC-003) |
