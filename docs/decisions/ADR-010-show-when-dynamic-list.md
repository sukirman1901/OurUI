# ADR-010: Show / When + dynamic List/Table
...
Apps need conditional UI and lists driven by `State`, without adopting a client VDOM (Reflex/React model). Dialog already proves the pattern: boolean State → bind → host attribute → CSS.
...
3. **`ui.List(items=State)` / `ui.Table(columns=static, rows=State)`** — initial SSR from State snapshot; `applyState` rebuilds `<li>` / `<tr>` from JSON arrays (strings or plain dicts). No nested `ui.*` item templates in v1.
...
- Nested component-per-row deferred; file upload deferred.
